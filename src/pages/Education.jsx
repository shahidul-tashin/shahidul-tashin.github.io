...
export default function Education() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-12 sm:px-6 sm:pt-16">
      ...
      {/* PMSCS detail card */}
      <section className="mt-10 rounded-2xl border border-base-300/60 bg-base-200/30 p-6 sm:p-8">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          {pmscs.semesters.map((sem) => (
            <div
              key={sem.label}
              className="rounded-xl border border-base-300/50 bg-base-100/60 p-4"
            >
      ...
      <section className="mt-16">
        ...
        <ul className="mt-6 space-y-0">
          {rest.map((item, i) => (
            <li key={item.degree} className="relative pb-8 pl-8 last:pb-0">
              {i !== rest.length - 1 && (
                <span className="absolute left-[7px] top-3 h-full w-px bg-base-300" />
              )}
              <span className="absolute left-0 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-primary bg-base-100" />
